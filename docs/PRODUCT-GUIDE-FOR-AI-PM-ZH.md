...
| `grammar family` | Rune Weaver 用来组织可复用机制边界的稳定分类，例如 `scheduler/timer`、`selection/draft`。它不是 case 名，也不是业务 feature 名。 |
| `capability` | family 下面最小的可复用机制原子。它表达“系统当前支持了哪种机制能力”，不直接表达完整业务故事。 |
| `composition` | 多个 capability 或多个 family 的组合行为。很多用户口中的“一个大语义”实际应该落在组合，而不是变成新的 grammar。 |
| `feature-owned source model` | 某个 feature 自己拥有的业务对象、catalog、配置与 update merge 输入。它属于 feature 的 authoring data，不等于 grammar 或 capability。 |
...
可以把这条链理解成一个顺序明确的产品化流程：

1. 需求描述  
2. 结构化意图（`IntentSchema`）  
3. 确定性结构（`FinalBlueprint`）  
4. `ModuleNeed` 驱动的 family / capability 匹配  
5. 宿主 realization 与 routing  
6. generation / review / validate  
7. 进入同一个 feature 的 lifecycle 管理
...
### family / capability / composition 分层

Rune Weaver 后续要维护的，不应该是越来越多的“业务 grammar”。

更稳的做法是三层分开：

1. `grammar family`  
   它负责回答“这是一类什么机制边界”，例如 `scheduler/timer`。

2. `capability`  
   它负责回答“这个 family 当前已经 admitted 的最小机制能力是什么”。

3. `composition`  
   它负责回答“一个用户口中的大语义，是不是多个 capability / family 的组合”。

当前真实例子是：

- `scheduler/timer` 是 family
- `timing.cooldown.local` 是当前已落地的 capability 示例
- `cooldownSeconds = 12` 这类字段才是参数
...
- 哪些词只是业务对象
- 哪些变化只是参数变化
- 哪些变化其实应该落在 feature 自己的 source model

### 为什么不能只靠 LLM 拆分 grammar

LLM 当然可以帮忙做 decomposition，它很适合：

- 提出 candidate family
- 提出 candidate capability
- 揭示一个需求其实由多个机制组成
- 指出哪里像参数变化、哪里像 source-model 变化

但 Rune Weaver 不能把这件事完全交给 LLM 临场决定。

原因不是“不信任模型”，而是系统里有几条链必须依赖稳定词汇：

- `ModuleNeed -> capability fit`
- pattern admission
- host routing
- honest block
- update safety

如果没有一套稳定 vocabulary，系统就会越来越依赖 prompt 表面词汇、历史 case 名和临时解释。
那样短期看很灵活，长期会让：

- 泛化边界飘移
- pattern 选择不稳定
- update / lifecycle 治理越来越难
- later agent 很难判断新语义该放在哪里

所以更合理的分工是：

- LLM 负责建议拆分
- deterministic grammar / family / capability vocabulary 负责最终约束和复用

### 一个最小链路

如果只用最短的话来解释 Rune Weaver 的上游链路，可以把它理解成：
...
4. admitted grammar  
5. `FinalBlueprint`  
6. `Pattern / Routing / Generation`

这条链里最容易被误解的地方是第 2 步和第 3、4 步。

`IntentSchema` 负责把自然语言需求整理成结构化意图，它可以附带：
...
平台在这里确实会做一次归一化与准入裁决，但它不是再来一个开放式的“自由语义理解器”。
它做的是更窄的事情：

- 折叠同义表达
- 检查字段是否齐全
- 把候选语义映射到平台已维护的稳定 vocabulary
- 判断当前版本是否真的 admitted 这条机制路径

只有进入 `admitted grammar` 的那部分，才会继续进入 `FinalBlueprint`，再去做 pattern 选择、宿主 routing 和 generation。

如果某个需求虽然被 LLM 理解到了，但当前平台没有 admitted 对应 grammar / capability，正确结果就应该是 honest block，而不是假装继续往下生成。

所以 Rune Weaver 不是让平台“理解两次需求”，而是把：

- 第一次开放世界理解交给 `IntentSchema`
- 把稳定执行约束交给 grammar admission

前者负责理解用户想做什么，后者负责约束系统今天真正承认自己能做什么。

### 一个组合例子

“怪物每 30 秒增强一次”听起来像一个大功能语义，但它不应该直接变成一个 capability 名。

更合理的理解方式是：

- `timing.periodic.local`
- `progress.counter.local`
- 再加效果或状态应用

这里的 `timing.periodic.local` 与 `progress.counter.local` 是说明组合方式的 vocabulary example，不表示它们今天都已经落地成 current shipped truth。

也就是说，它更像一个 composition，而不是一个新的 grammar family。

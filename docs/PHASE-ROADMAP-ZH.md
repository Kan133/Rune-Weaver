...
>
> 本文档用于解释 phase 模型。
> 当前实现真相、当前 MVP 边界、当前 agent 执行口径，请优先参考 [AGENT-EXECUTION-BASELINE.md](/D:/Rune%20Weaver/docs/AGENT-EXECUTION-BASELINE.md) 与 [HANDOFF.md](/D:/Rune%20Weaver/docs/HANDOFF.md)。
...
3. Phase 3：feature lifecycle 能不能成为第一公民并实现平台化
...
让 feature lifecycle 成为第一公民，实现从"受控可用"到"平台化、可治理、可规模化"的升级。

### 主要应该做什么

- 建立完整的 feature lifecycle：create / review / update / regenerate / rollback / history
- governance 从 review-layer baseline 升级为产品级边界管理
- host-aware scaling 成为显式设计纪律，而非事后适配
- 支持多 host 扩展的清晰结构
- 可选的 LangGraph 等基础设施仅用于支撑上述目标，不应成为阶段定义本身
- 保持 reviewability / rollback safety / host ownership boundary

### 验收标准

Phase 3 可以视为建立，至少要满足：

1. feature 的完整生命周期管理是第一优先
2. governance 整合进产品边界而非仅在 review 层
3. 添加新 host 时有清晰的扩展路径
4. 系统主要瓶颈从"系统不会做"转向"策略怎么选、质量怎么权衡"

...
`Wizard -> IntentSchema -> Blueprint Builder -> Pattern Resolution`
...
- `IntentSchema / Blueprint / AssemblyPlan` 这些对象已经是 Rune Weaver 自己的核心抽象
...
`Wizard`、`Blueprint Builder`、后续 `UI Wizard` 不应直接依赖：
- OpenAI SDK
- Anthropic SDK
- 厂商私有响应对象
...
因此抽象应围绕这些能力，而不是围绕 “OpenAI” 或 “Anthropic” 名字本身。
...
说明：
- `core/llm/types.ts`：统一接口
- `core/llm/errors.ts`：统一错误类型
- `core/llm/factory.ts`：环境变量工厂
- `providers/`：底层实现
...
`用 Rune Weaver 自己的统一 LLMClient 接口，底层只做薄 Provider 适配，优先支持 openai-compatible，通过 .env 读取 base_url / api_key / model，不把上层逻辑绑死在 LangChain、LangGraph 或某个厂商 SDK 上。`

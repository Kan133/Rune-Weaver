...
> Read when: mapping abstract assembly write targets onto the Dota2 host layout
> Do not use for: cross-host path policy, current product status, or universal write-target rules

This mapping is Dota2-specific and should be treated as a host reference, not a cross-host baseline.

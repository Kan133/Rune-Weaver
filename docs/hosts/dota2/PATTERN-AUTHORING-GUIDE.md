...
> Read when: extracting or authoring Dota2-oriented patterns from references or host implementations
> Do not use for: current PatternContract baseline wording, cross-host taxonomy authority, or live routing decisions

This guide remains useful for Dota2-specific pattern authoring, but cross-check newer `PatternContract` / `HostBinding` wording against the accepted baseline docs.

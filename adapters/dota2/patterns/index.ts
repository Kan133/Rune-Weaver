...
export const dota2Patterns: Dota2PatternMeta[] = [
  ...
  createDota2Pattern({
    ...
    category: "data",
    summary: "带权重的数据池",
    description: "管理带权重的数据集合，支持按权重随机抽取",
    responsibilities: [
    ...
    nonGoals: [
      { text: "不处理数据持久化", alternative: "使用 data.persistent_store" },
      { text: "不处理网络同步", alternative: "使用 data.networked_pool" },
    ],
    capabilities: ["weighted_selection", "tier_management", "random_draw"],
    inputs: [
      { name: "tiers", type: "string[]", required: true },
      { name: "weights", type: "Record<string, number>", required: false },
    ],
    outputs: [{ name: "selected", type: "any" }],
    parameters: [
    ...
    examples: [
      {
        name: "天赋抽取池",
        description: "带 R/SR/SSR/UR 稀有度的天赋池",
        params: { tiers: ["R", "SR", "SSR", "UR"], weights: { R: 50, SR: 30, SSR: 15, UR: 5 }, choiceCount: 3 },
        useCase: "三选一天赋系统",
      },
    ],
    hostTarget: "dota2.server",
  ...
  createDota2Pattern({
    ...
    category: "rule",
    summary: "多选一选择流程",
    description: "管理完整的多选一流程：展示候选项、接收选择、应用结果",
    responsibilities: [
    ...
    capabilities: ["multi_choice", "player_selection", "result_apply"],
    inputs: [
    ...
    dependencies: [
      { patternId: "data.weighted_pool", relation: "optional", reason: "用于获取候选项" },
      { patternId: "ui.selection_modal", relation: "optional", reason: "用于展示选择界面" },
    ],
    examples: [
      {
        name: "三选一天赋",
        description: "展示3个天赋供玩家选择",
  ...
  createDota2Pattern({
    ...
    category: "ui",
    summary: "选择弹窗",
    description: "模态弹窗展示多个选项供玩家选择，支持卡片布局和动画",
    responsibilities: [
    ...
    capabilities: ["multi_card_display", "player_selection", "animation", "dismiss"],
    inputs: [
    ...
    examples: [
      {
        name: "天赋选择",
        description: "横向排列的三个天赋卡",
        params: { choiceCount: 3, layoutPreset: "card_tray" },
        useCase: "天赋抽取系统",
      },
    ],

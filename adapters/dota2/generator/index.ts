...
function generateKeyBindingCode(className: string, featureId: string, entry: WritePlanEntry): string {
  return `import { GameMode } from "../../GameMode";
...
function generateDashEffectCode(className: string, featureId: string, entry: WritePlanEntry): string {
  return `import { BaseAbility } from "../../lib/dota_ts_adapter";
...
function generateUICode(
  ...
  switch (patternId) {
    ...
    case "ui.selection_modal":
      return generateSelectionModalComponent(componentName, featureId);
    case "ui.resource_bar":
 ...
 * 生成选择模态框组件
 */
function generateSelectionModalComponent(componentName: string, featureId: string): string {
  return `import React, { useState, useEffect } from "react";

...
export interface EnsureBridgeFilesResult {
  success: boolean;
  serverBridgeCreated: boolean;
  serverIndexCreated: boolean;
  uiBridgeCreated: boolean;
  uiIndexCreated: boolean;
  errors: string[];
}
...
const BRIDGE_PATHS = {
  ...
  hudStyles: "content/panorama/src/hud/styles.less",
...
export function ensureBridgeFiles(projectPath: string): EnsureBridgeFilesResult {
  const result: EnsureBridgeFilesResult = {
    success: false,
    serverBridgeCreated: false,
    serverIndexCreated: false,
    uiBridgeCreated: false,
    uiIndexCreated: false,
    errors: [],
  };

  const serverBridgePath = join(projectPath, BRIDGE_PATHS.serverBridge);
  const serverBridgeContent = `// Rune Weaver Server Bridge
...
`;
  ...
  try {
    if (!existsSync(serverBridgePath)) {
      writeFileSync(serverBridgePath, serverBridgeContent, "utf-8");
      result.serverBridgeCreated = true;
    } else {
    ...
    result.errors.push(
      `Failed to ensure server bridge: ${error instanceof Error ? error.message : String(error)}`
    );
  }

  const serverGeneratedIndexPath = join(projectPath, BRIDGE_PATHS.serverGeneratedIndex);
...
export function activateRwGeneratedServer(): void {
  ...
  print("[Rune Weaver] Generated server modules activated");
}
`;
  ...
  try {
    if (!existsSync(serverGeneratedIndexPath)) {
      ...
      result.serverIndexCreated = true;
    }
  } catch (error) {
    result.errors.push(
      `Failed to ensure server generated index: ${error instanceof Error ? error.message : String(error)}`
    );
  }

  const uiBridgePath = join(projectPath, BRIDGE_PATHS.uiBridge);
  const uiBridgeContent = `// Rune Weaver UI Bridge
...
`;
  ...
  try {
    if (!existsSync(uiBridgePath)) {
      writeFileSync(uiBridgePath, uiBridgeContent, "utf-8");
      result.uiBridgeCreated = true;
    } else {
      const existingContent = readFileSync(uiBridgePath, "utf-8");
      if (!existingContent.includes("RuneWeaverGeneratedUIRoot")) {
        writeFileSync(uiBridgePath, uiBridgeContent, "utf-8");
      }
    }
  } catch (error) {
    result.errors.push(
      `Failed to ensure UI bridge: ${error instanceof Error ? error.message : String(error)}`
    );
  }

  const uiGeneratedIndexPath = join(projectPath, BRIDGE_PATHS.uiGeneratedIndex);
...
`;
  ...
  try {
    if (!existsSync(uiGeneratedIndexPath)) {
      ...
      result.uiIndexCreated = true;
    }
  } catch (error) {
    result.errors.push(
      `Failed to ensure UI generated index: ${error instanceof Error ? error.message : String(error)}`
    );
  }
...
function generateServerIndexContent(
  ...
  const moduleDescriptors: Array<{
    fileName: string;
    runtimeNamespace: "rune_weaver.generated.server" | "rune_weaver.generated.shared";
    className: string;
    registerKey: string;
  }> = [];
  const generatedAbilityNames: string[] = [];
  const runtimeWiringPlans: Array<{
    featureId: string;
    keyBindingFile?: string;
    keyBindingClass?: string;
    poolFile?: string;
    poolClass?: string;
  ...
  for (const feature of activeFeatures) {
    ...
    for (const file of feature.generatedFiles) {
      const isServerTs = file.includes("/generated/server/") && file.endsWith(".ts");
      const isSharedTs = file.includes("/generated/shared/") && file.endsWith(".ts");
      if (!isServerTs && !isSharedTs) {
        continue;
      }
      ...
      const runtimeNamespace = isSharedTs
        ? "rune_weaver.generated.shared"
        : "rune_weaver.generated.server";
...
function loadRwGeneratedModules(): Record<string, any> {
  const loadedModules: Record<string, any> = {};

  for (const descriptor of moduleDescriptors) {
    const runtimePath = descriptor.runtimeNamespace + "." + descriptor.fileName;
    const mod = require(runtimePath);
    loadedModules[runtimePath] = mod;

    const registerFn = mod[descriptor.registerKey];
    if (registerFn && shouldRegisterModule(descriptor)) {
      registerFn();
    }
  }
...
function getRwSingleton(loadedModules: Record<string, any>, runtimePath: string, className: string): any {
  const mod = loadedModules[runtimePath];
  if (!mod) {
    return undefined;
  }
...
function wireRuntimeFeatures(loadedModules: Record<string, any>): void {
  for (const plan of runtimeWiringPlans) {
    if (!plan.keyBindingFile || !plan.keyBindingClass || !plan.poolFile || !plan.poolClass || !plan.selectionFlowFile || !plan.selectionFlowClass) {
      continue;
    }

    const keyBinding = getRwSingleton(
      loadedModules,
      "rune_weaver.generated.server." + plan.keyBindingFile,
      plan.keyBindingClass
    );
    const pool = getRwSingleton(
      loadedModules,
      "rune_weaver.generated.shared." + plan.poolFile,
      plan.poolClass
    );
    ...
    if (
      keyBinding &&
      typeof keyBinding.setHandler === "function" &&
      selectionFlow &&
      ...
      print("[Rune Weaver] Runtime wiring ready for feature " + plan.featureId);
    }
  }
}
...
function refreshHudStyleImports(projectPath: string, features: RuneWeaverFeatureRecord[]): void {
  ...
  for (const feature of activeFeatures) {
    for (const file of feature.generatedFiles) {
      if (!file.includes("panorama") || !file.endsWith(".less")) {
        continue;
      }
      const fileName = file.split("/").pop();
      if (fileName) {
        styleImports.add(`@import "../rune_weaver/generated/ui/${fileName}";`);
      }
    }
  }
...
export function refreshBridge(
  ...
  const ensureResult = ensureBridgeFiles(projectPath);
  if (!ensureResult.success) {
    result.errors.push(...ensureResult.errors);
  }
...
`;
  ...
  try {
    ...
    refreshHudStyleImports(projectPath, workspace.features);
